...
| v2 | v3 |
|----|-----|
| `value.as_time_components() -> Option<(NaiveDateTime, Option<FixedOffset>)>` | `value.as_datetime() -> Option<ExifDateTime>`（`ExifDateTime::Aware`/`Naive`，配 `aware()`/`into_naive()`/`or_offset(fallback)` 三个用法） |
| `value.as_u8array()` | `value.as_u8_slice()` |
| `value.to_u8array()` | （删除——使用 `as_u8_slice().map(<[u8]>::to_vec)`） |
| 缺少 `as_i64`/`as_f64`/`as_u16_slice`/... | 已补齐；`as_i8`/`as_i16`/`as_f32` 刻意不提供（现代 EXIF 几近不出现） |
...
2. **`PartialVec` 底层从 `Arc<Vec<u8>>` 切换到 `bytes::Bytes`**（P4.5）：内部存储类型变更，公开 API 零影响。`Bytes` 已是硬依赖；切换后享受 `try_into_mut` / `slice_ref` / `from_static` 等成熟 API，并为 §3.3 的内存数据源（P7）铺路。`PartialVec` 结构体保留（CR3 多块路径需要 `(full_alloc, view_range)` 二元组语义）。
...
9. ~~**内存数据源（`Vec<u8>` / `&[u8]` / `Bytes`）支持？**~~ **已决议**：v3 内提供 zero-copy 内存源 `MediaSource::from_bytes(impl Into<bytes::Bytes>)`。理由：`bytes::Bytes` 已是硬依赖，`PartialVec` 完成 `Bytes` 化（决议 8）后内存路径几乎是平凡的——`fill_buf` 变 no-op、`clear_and_skip` 变 `position += n`、share 直接 `Bytes::clone`。增量公开表面只多一个构造器，对 streaming 路径零侵入；面向 WASM / 移动端 / HTTP 代理服务等"字节已在内存里"的场景。落地于 P7（独立 phase，依赖 P4.5）。如 v3.0.0 cutover 范围紧张，可推迟到 v3.1 而不破坏其他承诺。
